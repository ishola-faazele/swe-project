...
describe('currency configuration', () => {
  it('defaults to NGN when NEXT_PUBLIC_CURRENCY is unset, without warning', async () => {
  ...
  it('honors a valid override (GHS) with that currency’s own locale conventions', async () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {})

    const { formatCurrency, getCurrencyCode, getCurrencySymbol } = await loadCurrencyModule('GHS')
  ...
  it('falls back to NGN and warns on a well-formed but UNASSIGNED code', async () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {})
  ...
  it('falls back to NGN and warns on a malformed code, without throwing', async () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {})
    ...
    const { getCurrencyCode } = await loadCurrencyModule('NGNN')

    expect(getCurrencyCode()).toBe('NGN')
    expect(warn).toHaveBeenCalledTimes(1)
    expect(warn.mock.calls[0][0]).toContain('NGNN')
  })
...
describe('getCurrencySymbol', () => {
  it('returns only the bare symbol, not a formatted amount', async () => {
    ...
    expect(symbol).toBe('₦')
    // Guards the form-label use case: "Total Price (₦)", never "Total Price (₦0.00)".
    expect(symbol).not.toMatch(/\d/)
  })
})

describe('BUSINESS_LOCALE', () => {
  it('is en-NG, the single place the locale choice is declared', async () => {
    const { BUSINESS_LOCALE } = await loadCurrencyModule(undefined)
    expect(BUSINESS_LOCALE).toBe('en-NG')
  })
})
