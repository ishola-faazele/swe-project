 ...
 * mock. `next/cache`'s `revalidatePath` is aliased to a no-op stub (see test/mocks/next-cache.ts)
 * since these actions run outside an actual Next.js request here; the DB work under test always
 * happens before that call. Every fixture is scoped and torn down per test — see
 * test/helpers/fixtures.ts.
 */
import { afterEach, beforeEach, describe, expect, it } from 'vitest'
import { prisma } from '@/lib/prisma'
import { cleanupFixtures, createCustomer, createInventoryItem, uniqueName } from '../../../../test/helpers/fixtures'
import { createDish, updateDish, deleteDish, toggleDishActive, getDishes } from './actions'
