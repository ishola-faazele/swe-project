 ...
 * recreate -> reapply) is the crux of this file. `next/cache` is aliased to a no-op stub (see
 * test/mocks/next-cache.ts); this action doesn't touch notifications, so nothing else is mocked.
 */
...
describe('updateOrderItems (integration)', () => {
  ...
  it('an item that drops out of the selection entirely reverts FULLY to baseline', async () => {
    ...
    await updateOrderItems(order.id, { dishes: [{ dishId: friedRice.id, quantity: 2 }], extraIngredients: [], totalPrice: 2600 })
    ...
    await updateOrderItems(order.id, { dishes: [], extraIngredients: [], totalPrice: 0 })
  ...
  it('re-snapshots dishName/unitPrice at edit time rather than carrying over the original order values', async () => {
    ...
    await updateOrderItems(order.id, { dishes: [{ dishId: dish.id, quantity: 1 }], extraIngredients: [], totalPrice: 1500 })
    ...
    await expect(
      updateOrderItems(order.id, {
        dishes: [
          ...
          { dishId: 'does-not-exist', quantity: 9 },
        ],
        extraIngredients: [],
        totalPrice: 1200,
      })
  ...
  it('updates order.totalPrice to the supplied value', async () => {
    ...
    await updateOrderItems(order.id, { dishes: [], extraIngredients: [], totalPrice: 999 })
