...
/**
 * CustomerLayout is a Client Component so it can own the sidebar-collapsed state
 * and pass it down to CustomerSidebar (which switches to icon-only mode when collapsed).
 *
 * Header is received as a render prop (`header`) so it stays a Server Component
 * — CustomerLayout must never import anything from utils/supabase/server directly.
 */
export function CustomerLayout({ children, header }: CustomerLayoutProps) {
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:block sticky top-0 h-screen overflow-y-auto">
        <CustomerSidebar collapsed={collapsed} onToggleCollapse={toggle} />
      </div>
